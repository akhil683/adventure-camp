  ...
  return (
    <div className='sm:max-w-[280px] w-full flex flex-col justify-between p-4 max-sm:px-8  rounded-lg hover:shadow-2xl hover:shadow-gray-300 duration-200 font-roboto md:h-[420px]'>
      ...
      <div className='flex flex-col justify-between md:h-[150px]'>
         ...
         <p className='md:text-xs text-sm text-gray-500'> {`${description?.slice(0,100)}...`}</p>
         <p className='font-bebasNeue text-2xl my-1'><strike className='text-lg text-gray-600 mr-2'>${price}</strike>${price}</p>
      </div>
      </Link>
      <button onClick={() => handleCart(product)} className='flex justify-center items-center gap-2 uppercase font-roboto w-full py-2 rounded-lg bg-green-600 hover:bg-red-600 duration-200 text-white'>
